...
public class PlatformRepositoryImpl implements CustomPlatformRepository{
    ...
    public Page<Platform> findByStatus(Pageable pageable, PlatformStatus status) {
        ...
        Long platformCount = query.select(QPlatform.platform.count()).from(QPlatform.platform).fetchFirst();
    ...
    public Page<Platform> findByQuery(String platformName, Pageable pageable, SortType sort) {
        List<Platform> platforms = query.select(QPlatform.platform).from(QPlatform.platform)
                .where(QPlatform.platform.status.eq(PlatformStatus.ACCEPT).and(QPlatform.platform.name.contains(platformName)))
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
    ...
    private OrderSpecifier<?> sortConverter(SortType sort) {
        ...
        switch (sort) {
            case STAR_ASC ->  { return new OrderSpecifier(Order.ASC, QPlatform.platform.star); }
            case STAR_DESC ->  { return new OrderSpecifier(Order.DESC, QPlatform.platform.star); }


            case DATE_DESC -> { return new OrderSpecifier(Order.DESC, QPlatform.platform.createdDt); }
            default ->  { return new OrderSpecifier(Order.ASC, QPlatform.platform.createdDt); }
        }
